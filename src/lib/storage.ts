...
import { formatPeruDate } from './time';
...
export const storage = {
  ...
  getDailyStats: (date: string): { newMemberships: any[], newPayments: any[] } => {
    ...
    const newMemberships = clients.filter(c => {
      ...
      const createdDate = formatPeruDate(c.created_at, 'yyyy-MM-dd');
      if (createdDate === targetDate) return true;
      
      const hasRenewalToday = actionHistory.some(ah => 
        ah.client_id === c.id && 
        ah.action_type === 'Renovación' && 
        formatPeruDate(ah.date, 'yyyy-MM-dd') === targetDate
      );
      return hasRenewalToday;
    }).map(c => {
      const initialOrRenewalPayment = payments.find(p => 
        p.client_id === c.id && 
        (p.type === 'Initial' || p.type === 'Renewal') &&
        formatPeruDate(p.date, 'yyyy-MM-dd') === targetDate
      );
      
      return {
        ...c,
        amount_paid: initialOrRenewalPayment ? initialOrRenewalPayment.amount : 0,
        payment_method: initialOrRenewalPayment ? initialOrRenewalPayment.method : c.payment_method,
        event_date: initialOrRenewalPayment ? initialOrRenewalPayment.date : c.created_at
      };
    });

    const newPayments = payments.filter(p => {
      if (p.is_imported || p.type === 'Initial' || p.type === 'Renewal') return false;
      const paymentDate = formatPeruDate(p.date, 'yyyy-MM-dd');
  ...
  getConsolidatedStats: (filter: string): any[] => {
    ...
    const isMatch = (dateStr: string) => {
      ...
      const datePeru = formatPeruDate(dateStr, 'yyyy-MM-dd');
      const nowPeru = formatPeruDate(new Date(), 'yyyy-MM-dd');
      
      if (filter === 'day') {
        return datePeru === nowPeru;
      } else if (filter === 'month') {
        return datePeru.substring(0, 7) === nowPeru.substring(0, 7);
      } else if (filter === 'quarter') {
        const dateMonth = parseInt(datePeru.substring(5, 7), 10) - 1;
        const nowMonth = parseInt(nowPeru.substring(5, 7), 10) - 1;
        const dateYear = parseInt(datePeru.substring(0, 4), 10);
        const nowYear = parseInt(nowPeru.substring(0, 4), 10);
        
        const currentQuarter = Math.floor(nowMonth / 3);
        const dateQuarter = Math.floor(dateMonth / 3);
        
        return dateYear === nowYear && dateQuarter === currentQuarter;
      } else if (filter === 'year') {
        return datePeru.substring(0, 4) === nowPeru.substring(0, 4);
      }
      return true;
    };
    ...
    const newClientsEvents = clients
      ...
      .map(c => {
        const initialPayment = payments.find(p => p.client_id === c.id && p.type === 'Initial');
        return {
          id: c.id,
          membership_type: c.membership_type,
          classification: c.classification,
          payment_method: initialPayment ? initialPayment.method : c.payment_method,
          amount_paid: initialPayment ? initialPayment.amount : 0,
          price: c.price,
    ...
    const debtEvents = payments
      .filter(p => p.type === 'Debt' && !p.is_imported && isMatch(p.date))
      .map(p => {
        const c = clients.find(client => client.id === p.client_id);
        if (!c) return null;
        return {
    ...
    return [...newClientsEvents, ...renewalEvents, ...debtEvents];
  }
};
