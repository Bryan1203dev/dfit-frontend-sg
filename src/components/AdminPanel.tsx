...
import { Search, Edit, Trash2, UserCheck, PartyPopper, X, Save, Calendar, Plus, RefreshCw } from 'lucide-react';
        ...
        <div className="flex flex-wrap gap-4 items-center">
            <div className="flex items-center gap-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-1.5 shadow-sm">
                ...
                <input 
                    ...
                    className="bg-transparent outline-none text-sm text-zinc-900 dark:text-white"
                    title="Fecha inicio"
                />
                ...
                {(filterStartDate || filterEndDate) && (
                    <button onClick={() => { setFilterStartDate(''); setFilterEndDate(''); }} className="text-zinc-400 hover:text-red-500 ml-1">
                        <X className="w-4 h-4" />
                    </button>
                )}
            </div>
        ...
        {isWelcomeOpen && selectedClient && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <motion.div 
              ...
              className="bg-zinc-900 text-white w-full max-w-md rounded-3xl shadow-2xl border border-amber-500/30 overflow-hidden text-center relative"
            >
                <button onClick={() => setIsWelcomeOpen(false)} className="absolute top-4 right-4 text-zinc-500 hover:text-white"><X /></button>
                <div className="p-8">
                    <div className="w-20 h-20 bg-amber-500 rounded-full mx-auto mb-6 flex items-center justify-center shadow-[0_0_30px_rgba(245,158,11,0.5)]">
                        <PartyPopper className="w-10 h-10 text-white" />
                    </div>
                    <h2 className="text-3xl font-bold mb-2">¡Bienvenido a DFIT!</h2>
                    <p className="text-amber-500 font-mono text-xl mb-6">{selectedClient.full_name}</p>
                    
                    <div className="bg-zinc-800/50 rounded-xl p-4 mb-6 space-y-2">
                        <div className="flex justify-between">
                            <span className="opacity-60">Código:</span>
                            <span className="font-mono font-bold">{selectedClient.code}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="opacity-60">Membresía:</span>
                            <span className="font-bold">{selectedClient.membership_type}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="opacity-60">Fecha Inicio:</span>
                            <span className="font-bold">{formatPeruDate(selectedClient.start_date, 'dd/MM/yyyy')}</span>
                        </div>
                    </div>

                    <p className="text-sm opacity-80 leading-relaxed">
                        Gracias por ser parte de la familia DFIT. Nuestro equipo estará cerca de ti para ayudarte a cumplir todos tus objetivos.
                    </p>
                </div>
