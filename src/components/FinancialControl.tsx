...
export default function FinancialControl() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const currentDate = getCurrentPeruISODate();
      if (currentDate !== date) {
        setDate(currentDate);
      } else {
        fetchStats();
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-4 items-center w-full sm:w-auto">
            <button 
                onClick={exportExcel}
                className="flex items-center justify-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-500 transition-colors w-full sm:w-auto"
            >
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
              {data.newMemberships.map((item: any) => (
                <tr key={item.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                  <td className="px-6 py-3 font-mono opacity-60">{formatPeruDate(item.event_date || item.created_at, 'HH:mm')}</td>
