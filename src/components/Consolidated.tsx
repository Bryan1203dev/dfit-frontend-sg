...
export default function Consolidated() {
  ...
  // Process Data
  const membershipEvents = clients.filter(c => c.event_type === 'New' || c.event_type === 'Renewal');

  const membershipData = [
    { name: 'Diario', value: membershipEvents.filter(c => c.membership_type === 'Diario').length },
    { name: 'Interdiario', value: membershipEvents.filter(c => c.membership_type === 'Interdiario').length },
  ];

  const classificationData = membershipEvents.reduce((acc: any[], curr) => {
  ...
  const lineChartDataMap = clients.reduce((acc: any, curr) => {
    ...
    acc[key].deudas += curr.event_type === 'Debt' ? -(curr.amount_paid || 0) : Math.max(0, curr.price - (curr.amount_paid || 0));
    return acc;
  }, {});
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                <select 
                    ...
                    className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2 outline-none text-zinc-900 dark:text-white w-full sm:w-auto"
                >
                ...
                <button onClick={exportExcel} className="flex items-center justify-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-500 w-full sm:w-auto">
